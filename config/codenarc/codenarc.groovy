ruleset {
    ...
    ParameterAssignmentInFilterClosure(priority: 1)
    ...
    CouldBeSwitchStatement(priority: 3)
    FieldTypeRequired(priority: 2)
    HashtableIsObsolete(priority: 1)
    ImplicitReturnStatement(priority: 2)
    InvertedCondition(priority: 1)
    InvertedIfElse(priority: 1)
    MethodParameterTypeRequired(priority: 1)
    LongLiteralWithLowerCaseL(priority: 1)
    MethodReturnTypeRequired(priority: 2, doNotApplyToClassNames: '*Spec,*Specification')
    NoDef(priority: 3, doNotApplyToClassNames: '*Spec,*Specification')
    NoJavaUtilDate(priority: 1)
    NoTabCharacter(priority: 1)
    ParameterReassignment(priority: 3)
    StaticFieldsBeforeInstanceFields(priority: 1)
    TernaryCouldBeElvis(priority: 1)
    TrailingComma(priority: 2)
    VectorIsObsolete(priority: 1)

    // rulesets/design.xml
    AbstractClassWithPublicConstructor(priority: 3)
    AbstractClassWithoutAbstractMethod(priority: 3, doNotApplyToClassNames: '*Specification')
...
//    CloneableWithoutClone(priority: 1)
    ...
    OptionalCollectionReturnType(priority: 1)
    OptionalField(priority: 2)
...
//    CloneWithoutCloneable(priority: 1) TODO: Issue with Java 21 temporary disable it
//    JUnitAssertEqualsConstantActualValue(priority: 1) TODO: Issue with Java 21 temporary disable it
//    UnsafeImplementationAsMap(priority: 1) TODO: Issue with Java 21 temporary disable it
//    MissingOverrideAnnotation(priority: 1) TODO: Issue with Java 21 temporary disable it
    ...
    BlockEndsWithBlankLine(priority: 1)
    BlockStartsWithBlankLine(priority: 1)
    ...
    ClassStartsWithBlankLine(priority: 1)
    ClosureStatementOnOpeningLineOfMultipleLineClosure(priority: 1)
    ConsecutiveBlankLines(priority: 1)
    FileEndsWithoutNewline(priority: 1)
    ...
    MissingBlankLineBeforeAnnotatedField(priority: 1)
    SpaceAfterMethodDeclarationName(priority: 1)
    SpaceAfterMethodCallName(priority: 1)
    ...
    SpaceAfterNotOperator(priority: 1)
    ...
    SpaceAroundOperator(priority: 1)
    SpaceBeforeClosingBrace(priority: 3)
    SpaceBeforeOpeningBrace(priority: 3)
    SpaceInsideParentheses(priority: 2)
    TrailingWhitespace(priority: 1)
    ...
    ExplicitCallToGetAtMethod(priority: 2)
    ...
    MethodName(priority: 1, doNotApplyToClassNames: '*Spec,*Specification')
    ...
    VariableName(priority: 1, finalRegex: null)
    ...
    // rulesets/size.xml
    ClassSize(priority: 3, maxLines: 500, name: 'ClassSizeWarning', description: 'A class that is 500 lines long is growing a bit fat, consider refactoring the logic into smaller classes with more focused responsibilities.')
    ...
    MethodCount(priority: 3, maxMethods: 20, name: 'MethodCountWarning', description: 'A class with 20 methods is probably a good suspect for refactoring.')
    MethodCount(priority: 1, maxMethods: 30, name: 'MethodCountError', description: 'A class with 30 methods must be refactored!')
    ...
    MethodSize(priority: 1, maxLines: 80, ignoreMethodNames: 'equals, hashCode', name: 'MethodSizeError', description: 'A 80 line method is too long, refactor it!', doNotApplyToClassNames: '*Spec')
    ...
    UnnecessaryBooleanExpression(priority: 1, doNotApplyToClassNames: '*Spec,*Specification')
    ...
    IllegalRegex(regex: /\/\*[^\*]/, violationMessage: 'Block comments found, please remove the comment', name: 'BlockComment', description: 'This check ensures that no block comments allowed to be present in code.', priority: 1)
}
